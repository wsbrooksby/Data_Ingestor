...
-- 	- There are some duplicate records in the data, and the ETL script should find and remove them when populating the database.
-- */
-- select
...
-- 	from artist as a
--         left join (select artist_id, count(artist_id) as genre_count from genre_artist
...
-- /*
-- Get cases where one artist_id is associated with the same genre name more than once in the genre_artist table.
-- 	- In this sample set, there were no duplicates based off of genre_id and artist_id
-- */
-- with cte2 as (
-- 	select 
-- 		ga.artist_id,
--         g.`name` as genre_name,
-- 		row_number() over(partition by ga.artist_id, g.`name`) as num
-- 	from genre as g
--     join genre_artist as ga on g.genre_id = ga.artist_id
-- )
-- select 
-- 	artist_id,
-- 	genre_name,
...
select 
    ...
    group_concat(parent_id order by lvl separator " --> ") as all_parents
from cte3
group by genre_id
having all_parents is not null
order by genre_name;